...
def test_retrieve_pokemons_by_type(grass_type_pokemons, water_type_pokemon):
    # criamos um arquivo em memória que o seu conteúdo são os dois pokemons
    fake_file = StringIO(
      json.dumps({"results": [grass_type_pokemons, water_type_pokemon]})
    )
    # quando pesquisamos por pokemons do tipo grama,
    # o pokemon do tipo água não deve ser retornado
    assert grass_type_pokemons in retrieve_pokemons_by_type("Grass", fake_file)
